...
void SceneBasic::readData(const char* fname)
{
    ...
    if (!ifs) {
        ...
        for (int i=0; i<27; i++) ifs >> positionData[i];
        for (int i=0; i<27; i++) ifs >> colorData[i];
    }
}
...
void SceneBasic::CreateVBO()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 27 * sizeof(float), positionData, GL_STATIC_DRAW);

     // bind colorBufferHandle to GL_ARRAY_BUFFER target
    glBindBuffer(GL_ARRAY_BUFFER, colorBufferHandle);
    glBufferData(GL_ARRAY_BUFFER, 27 * sizeof(float), colorData, GL_STATIC_DRAW);
